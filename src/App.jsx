...
function MSTAkademi({ yazarAdi, yazarUnvan, yazarId, onKapat, onIlerlemeGuncelle }) {
  ...
  const kayitEt = (yeniMod, yeniAsama, yeniTamamlananlar, yeniTestCevaplari) => {
    ...
    // Backend'e bildir (panel için)
    try {
      const tok = localStorage.getItem('mst_token');
      if (tok) fetch(`${BACKEND_URL}/api/author/akademi-ilerleme`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${tok}` },
        body: JSON.stringify({ mod: yeniMod, asama: yeniAsama, tamamlananlar: yeniTamamlananlar, testCevaplari: yeniTestCevaplari || testCevaplari }),
      }).catch(() => {});
    } catch {}
...
function BeklemeIcerigi({ eserAdi, yazarAdi, ilerleme, simAdim }) {
  ...
  const testCevap = (key, val) => {
    ...
    setTestCvp(yeni);
    // İkna profili için test cevaplarını kaydet
    const yeniTestCevaplari = { ...testCevaplari, [key]: val };
    setTestCevaplari(yeniTestCevaplari);
    kayitEt(aktifMod, aktifAsama, tamamlananlar, yeniTestCevaplari);
  ...
  if (akademiAcik) return <MSTAkademi yazarAdi={yazarAdi} yazarUnvan={oturum?.unvan} yazarId={oturum?.id} onKapat={() => setAkademiAcik(false)} />;
