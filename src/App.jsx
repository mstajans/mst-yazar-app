  ...
  const gonder = async () => {
    ...
    try {
      const d = await api("/api/author/reklam/kampanya", {
        method: "POST",
        body: JSON.stringify({ bookId: Number(bookId), butce: Number(butce), gun: Number(gun), kitle, amac }),
      });
          ...
          {liste.map((k) => {
            ...
            return (
              <div key={k.id} style={{ background: "rgba(12,23,48,.55)", padding: "22px 24px" }}>
                ...
                {d.not && <div style={{ fontSize: 13, color: "rgba(245,240,228,.58)", marginTop: 8, fontWeight: 300 }}>{d.not}</div>}
...
function MSTAkademi({ yazarAdi, onKapat }) {
  ...
  const geribildiri = async () => {
    ...
    if (satisMi) {
      ...
      metin = `Çok iyi! Bu yanıt doğru bir bakış açısı yansıtıyor. Somut, uygulanabilir ve ${ilkAd} Bey'in durumuna uygun. Bu yaklaşımı tutarlı şekilde sürdürün.`;
    ...
    setYukl(false);
    setAiGoster(true);
    if (!tamamlananlar.includes(aktifMod)) setTamamlananlar(t => [...t, aktifMod]);
  };
          ...
          <div className="ak-blok">
            ...
            <div className="ak-tamam">
              ...
              <div>
                ...
                <div className="ak-tamam-m">{aktifMod < 9 ? `Yarın Modül ${mod.no+1} açılıyor.` : 'Tebrikler! Temel seviyeyi tamamladınız.'}</div>
              </div>
            </div>
            {aktifMod < 9 ? (
...
function BeklemeIcerigi({ eserAdi, yazarAdi, ilerleme, simAdim }) {
  ...
  if (akademiAcik) return <MSTAkademi yazarAdi={yazarAdi} onKapat={() => setAkademiAcik(false)} />;
      ...
      <div className="bk-scroll" ref={scrollRef}>
        ...
        <div className="bk-bolum" style={{ background: "#0A1628" }}>
          <div className="bk-ic">
            ...
            <div className="bk-hitap">Beklerken öğrenin — her gün 1 modül açılıyor.</div>
            <h2 className="bk-bas" style={{marginBottom:20}}>10 modül, 10 gün, profesyonel yazar</h2>
            <p className="bk-metin" style={{marginBottom:32}}>Sosyal medya stratejisi, lansman planlaması, okur topluluğu oluşturma, medya görünürlüğü — kitabınız yayınlanmadan önce her şeyi öğrenin. Her modül gerçek senaryolar, yanlış/doğru analizi ve AI geri bildirimi içeriyor.</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:28}}>
              {MODULLER_DATA.slice(0,4).map((m,i) => (
                <div key={i} style={{border:"1px solid rgba(201,162,75,.15)",padding:"16px 18px",background:"rgba(10,22,40,.4)"}}>
                  <div style={{fontSize:9,letterSpacing:".22em",color:"rgba(201,162,75,.5)",marginBottom:7}}>{i+1}. GÜN · MODÜL {i+1}</div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:17,color:"#F0D68A",lineHeight:1.3}}>{m.bas}</div>
                </div>
              ))}
            </div>
            <div style={{fontSize:12,color:"rgba(245,240,228,.35)",marginBottom:24,letterSpacing:".08em"}}>+ 6 modül daha: platform seçimi, video içerik, basın görünürlüğü, okur yorumu, veri analizi ve daha fazlası</div>
...
function AkademiKutlama({ adSoyad, onKapat }) {
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', maxWidth: 480, padding: '0 24px', animation: 'kutGel .8s both .2s' }}>
        ...
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 'clamp(28px,5vw,44px)', fontWeight: 300, color: '#F0D68A', lineHeight: 1.2, marginBottom: 16, animation: 'kutGel .8s both .6s' }}>
          {ilkAd} Bey,<br />akademiniz açıldı.
        </div>
        <p style={{ fontFamily: 'Jost,sans-serif', fontWeight: 300, fontSize: 15, color: 'rgba(245,240,228,.65)', lineHeight: 1.85, marginBottom: 36, animation: 'kutGel .7s both .75s' }}>
          MST Yazarlık Akademisi'ne hoş geldiniz. Her gün 1 modül açılacak, 10 günde profesyonel yayıncılık dünyasını öğrenmiş olacaksınız. Bu eğitim size özel ve tamamen ücretsiz.
        </p>
        {/* Modül önizleme */}
...
function YazarUygulamasi() {
  ...
  return (
    <div style={{
      ...
      {akademiKutlama && <AkademiKutlama adSoyad={account && account.name} onKapat={() => { setAkademiKutlama(false); setAkademiAcik(true); setTab("akademi"); }} />}
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "30px clamp(16px, 4vw, 40px) 130px", width: "100%", boxSizing: "border-box" }}>
        ...
        {tab === "stok" && !akademiAcik && (
          <div onClick={akademiAc} style={{
            ...
            <style>{`.ak-widget-cizgi{position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#C9A24B,transparent);animation:adaySur 2s ease both}`}</style>
            <div className="ak-widget-cizgi" />
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              ...
              <div style={{width:42,height:42,border:'1px solid rgba(201,162,75,.5)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:18,color:'#C9A24B'}}>→</div>
            </div>
        ...
        {tab === "akademi" && (
          akademiAcik
            ? <MSTAkademi yazarAdi={account.name} onKapat={() => { setAkademiAcik(false); setTab("stok"); }} />
            : <div style={{padding:40,textAlign:'center'}}>
                <button onClick={akademiAc} style={{padding:'14px 32px',background:'linear-gradient(135deg,rgba(140,106,34,.9),rgba(201,162,75,.85))',border:'none',color:'#050D1A',fontFamily:'Jost,sans-serif',fontSize:11,fontWeight:600,letterSpacing:'.2em',cursor:'pointer'}}>AKADEMİYE GİR →</button>
              </div>
        )}
